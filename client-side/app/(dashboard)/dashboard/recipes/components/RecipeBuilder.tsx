...
export default function RecipeBuilder({ mode }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 w-full max-w-5xl mx-auto">
      {/* Header */}
      <div className="bg-linear-to-r from-indigo-100 via-white to-purple-100 p-6 rounded-xl shadow space-y-4 border border-indigo-200 w-full">
        <div className="flex justify-between items-center">
          <h2 className="text-3xl font-extrabold text-indigo-700 tracking-tight">Recipe Builder</h2>
      ...
      <div className="rounded-2xl shadow-xl overflow-hidden border border-gray-200 w-full bg-linear-to-br from-indigo-50 via-white to-purple-50">
        ...
        <div className="divide-y">
          {fields.map((field, index) => {
            ...
            return (
              <div key={field.id} className="grid grid-cols-6 gap-2 px-6 py-4 items-center bg-white/80 hover:bg-indigo-50 transition rounded-xl shadow-sm">
                <input
      ...
      <div className="bg-linear-to-r from-indigo-100 via-white to-purple-100 p-4 rounded-xl shadow flex flex-col md:flex-row justify-between items-center border border-indigo-200 w-full">
        <div className="text-xl font-bold text-indigo-700 mb-2 md:mb-0">
          Total Cost: <span className="text-purple-700">Rp {totalCost.toLocaleString("id-ID")}</span>
        </div>
        <button
          type="submit"
