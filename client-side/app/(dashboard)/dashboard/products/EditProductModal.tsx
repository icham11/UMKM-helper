...
export default function EditProductModal({ product, categories, onClose, onSaved }: EditProductModalProps) {
  ...
  useEffect(() => {
    getIngredientOptions().then((opts) => setIngredientOptions(opts)).catch(() => {});
  }, []);
  ...
  const updateRow = (index: number, updated: DraftRecipeRowWithClientId) => setRecipe((prev) => prev.map((r, i) => (i === index ? updated : r)));
  const removeRow = (index: number) => setRecipe((prev) => prev.filter((_, i) => i !== index));
  const addRow = () => setRecipe((prev) => [...prev, {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-linear-to-r from-yellow-50 to-indigo-50">
          <h2 className="text-lg font-extrabold text-yellow-700">Edit Produk</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <div className="text-xs text-gray-400 mt-1">
                Biaya resep: {recipeCost > 0 ? new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(recipeCost) : "—"}
                {margin !== null && (
                  <>
                    {" — "}
                    <span className={
