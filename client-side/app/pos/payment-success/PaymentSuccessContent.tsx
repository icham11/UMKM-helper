...
export default function PaymentSuccessContent() {
      ...
      if (salesRes.ok) {
        ...
        if (salesJson.success && salesJson.data?.sales) {
          const found = salesJson.data.sales.find(
            (s: SaleData) =>
              s.id === Number(saleId) || s.transactionNumber === orderId
          );
          if (found) {
            setSaleData(found);
            return found;
  ...
  const handlePrintInvoice = async () => {
    ...
    try {
      const response = await fetch(
        `/api/sales/${saleId}/invoice?format=html`
      );
      if (!response.ok) {
  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      const response = await fetch(
        `/api/sales/${saleId}/invoice?format=html&download=true`
      );
      if (!response.ok) {
  ...
  const actualStatus = midtransSuccess
    ...
    : saleData?.paymentStatus ||
      (isKasbon ? "Kasbon" : isPendingParam ? "Pending" : "Paid");
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-8">
          {isPaid ? (
            ...
            <>
              ...
              <p className="text-gray-600">
                Transaksi dibuat — menunggu konfirmasi pembayaran dari Midtrans
              </p>
              <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-left">
                ...
                <p className="text-xs text-yellow-700 mt-1">
                  Stok bahan baku belum dipotong. Halaman ini akan otomatis
                  terupdate saat pembayaran berhasil. Anda juga bisa cek status
                  di halaman Sales History.
                </p>
              </div>
            </>
          ) : isKasbon ? (
            <>
              ...
              <p className="text-gray-600">
                Transaksi dicatat sebagai piutang pelanggan
              </p>
            </>
          ) : (
            <>
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-gray-900">
                  {saleData
                    ? paymentMethodLabel[saleData.paymentMethod] ||
            ...
            <div className="border-t border-gray-200 pt-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-extrabold text-indigo-600">
                  {saleData
                    ? formatRupiah(Number(saleData.totalRevenue))
                    : "—"}
                </span>
              </div>
            </div>
          ...
          {isPending && isMidtrans && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-center">
              ...
              <p className="text-yellow-600 text-sm">
                Invoice akan tersedia setelah pembayaran dikonfirmasi. Cek
                status di Sales History.
              </p>
            </div>
          )}
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <p className="text-sm text-blue-900">
            💡 <strong>Tips:</strong> Anda dapat mencetak atau mengunduh invoice
            kapan saja dari halaman riwayat penjualan.
          </p>
        </div>
      </div>
    </div>
